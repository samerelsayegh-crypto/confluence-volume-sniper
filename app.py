...
page = st.sidebar.radio("Navigation", ["Home Dashboard", "Market Analytics", "Stock Analysis"])
            ...
            with st.spinner(f"Fetching and analyzing multi-timeframe data for {analyze_symbol}..."):
                ...
                for tf in timeframes:
                    try:
                        ...
                        req = StockBarsRequest(
                            symbol_or_symbols=analyze_symbol,
                            timeframe=alpaca_tf,
                            start=start_dt,
                ...
                for i, tf in enumerate(timeframes):
                    if not results[tf].get("error"):
                        p_cols[i].metric(f"{analyze_symbol} Close Price ({tf})", f"${results[tf]['price']:.2f}")
                ...
                for i, tf in enumerate(timeframes):
                    with tabs[i]:
                        if results[tf].get("error"):
                            ...
                            fig.update_layout(
                                title=f"{analyze_symbol} - {tf} Chart with Moving Averages",
                                xaxis_title="Time",
                                yaxis_title="Price",
                                template="plotly_white",
                                height=500,
        ...
        if st.button("Detect ORB Signals", type="primary", key="btn_sig2"):
            with st.spinner(f"Analyzing {analyze_symbol} 5m footprint..."):
                try:
                    ...
                    st.markdown(f"### {analyze_symbol} ORB Levels ({latest_day})")
                    c1, c2 = st.columns(2)
        ...
        if st.button("Calculate VWAP", type="primary", key="btn_sig3"):
            with st.spinner(f"Analyzing {analyze_symbol} VWAP..."):
                try:
                    ...
                    st.markdown(f"### {analyze_symbol} VWAP Status ({latest_day})")
                    c1, c2, c3 = st.columns(3)
        ...
        if st.button("Calculate PM Range", type="primary", key="btn_sig5"):
            with st.spinner(f"Fetching extended hours data for {analyze_symbol}..."):
                try:
                    ...
                    if pm_data.empty:
                        st.warning(f"No Pre-Market data recorded for {analyze_symbol} on {latest_day}.")
                        st.stop()
                    ...
                    yf_ticker = yf.Ticker(analyze_symbol)
